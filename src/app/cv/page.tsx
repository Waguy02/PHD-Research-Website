...
const typeConfig: Record<string, { dot: string; badge: string }> = {
  paper: { dot: "border-blue-300 bg-blue-50 dark:border-blue-700 dark:bg-blue-900/40", badge: "bg-blue-50 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300" },
  education: { dot: "border-emerald-300 bg-emerald-50 dark:border-emerald-700 dark:bg-emerald-900/40", badge: "bg-emerald-50 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400" },
  milestone: { dot: "border-amber-300 bg-amber-50 dark:border-amber-700 dark:bg-amber-900/40", badge: "bg-amber-50 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400" },
  presentation: { dot: "border-purple-300 bg-purple-50 dark:border-purple-700 dark:bg-purple-900/40", badge: "bg-purple-50 text-purple-700 dark:bg-purple-900/40 dark:text-purple-400" },
};
    ...
    <div className="mx-auto max-w-4xl px-6 py-16">
      ...
      <section className="mb-16">
        ...
        <div className="relative">
          <div className="absolute left-[19px] top-2 h-[calc(100%-1.5rem)] w-px bg-gray-200 dark:bg-slate-800" />
            ...
            {timelineEvents.map((event, i) => {
              ...
              return (
                <div key={i} className="relative flex gap-5">
                  ...
                  <div className="flex-1 rounded-xl border border-gray-200 bg-white p-5 card-hover dark:border-slate-800 dark:bg-slate-900">
      ...
      <section className="mb-16">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {[
            ...
            <div key={group.title} className="rounded-xl border border-gray-200 bg-gray-50 p-5 dark:border-slate-800 dark:bg-slate-800/60">
              <h3 className="mb-3 font-semibold text-gray-900 dark:text-slate-100">{group.title}</h3>
              <div className="flex flex-wrap gap-2">
                {group.items.map((item) => (
                  <span key={item} className="rounded-lg border border-gray-200 bg-white px-2.5 py-1 text-xs text-gray-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">
                    {item}
                  </span>
                ))}
              </div>
      ...
      <section>
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-gray-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900">
            ...
            <p className="mb-2 text-sm text-gray-500 dark:text-slate-400">LORIA, CNRS, Universite de Lorraine / Forvis Mazars</p>
